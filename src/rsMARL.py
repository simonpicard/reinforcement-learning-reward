...
class rsMARL:
    def __init__(
        self, w, epsilon, gamma, alpha, lambd, start, flagShape, initialQ, runs, coop
    ):
    ...
    def run(self):
        Qas1 = [
            [
                [[self.initialQ] * 4 for k in range(2 ** 6)]
        ...
        for run in range(self.runs):
            ...
            while not self.isDone():
                ...
                if not self.world.isOnGoal(self.agent1) and not done1:
                    self.agent1, Qas1, action1, p1, done1, flagIndex1 = self.runAgent(
                        self.agent1, Qas1, path1, action1, 1, flagIndex1
                    )
                    self.action1 = action1
                    self.done1 = done1

                if not self.world.isOnGoal(self.agent2) and not done2:
                    self.agent2, Qas2, action2, p2, done2, flagIndex2 = self.runAgent(
                        self.agent2, Qas2, path2, action2, 2, flagIndex2
                    )
                    self.action2 = action2
                    self.done2 = done2
    ...
    def printShit(self, pos, agent):
        return (
            self.getStepPlan(agent, pos, "Join"),
    ...
    def runAgent(self, pos, Qas, path, action, agent, flagIndex):
        ...
        sigma, nextAction, p, nextFlagIndex = self.getSigma(
            reward, nextPos, pos, action, Qas, agent, flagIndex
        )
    ...
    def getSigma(self, reward, nextPos, currentPos, action, Qas, agent, flagIndex):
        ...
        sigma = (
            ...
            + (self.gamma * nextQ)
            - originalQ
            + self.rewardShaping(currentPos, nextPos, agent)
        )
