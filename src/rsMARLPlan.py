...
class rsMARL:
    ...
    def run(self):
        Qas1 = [
            [
                [
                    [[self.initialQ] * 4 for h in range(len(self.plan1))]
                    for k in range(2 ** 6)
                ]
        ...
        Qas2 = [
            [
                [
                    [[self.initialQ] * 4 for h in range(len(self.plan2))]
                    for k in range(2 ** 6)
                ]
                for i in range(self.world.size[0])
            ]
        ...
        for run in range(self.runs):
            ...
            self.action2 = 0
            self.action1 = 0
            action1, p1 = self.chooseAction(
                self.agent1, Qas1, 1, flagIndex1, planIndex1
            )
            action2, p2 = self.chooseAction(
                self.agent2, Qas2, 2, flagIndex2, planIndex2
            )
            ...
            while not self.isDone():
                ...
                if not self.world.isOnGoal(self.agent1) and not done1:
                    (
                        ...
                        self.agent1, Qas1, path1, action1, 1, flagIndex1, planIndex1
                    )
                    self.action1 = action1
                    self.done1 = done1
                ...
                if not self.world.isOnGoal(self.agent2) and not done2:
                    (
                        ...
                        self.agent2, Qas2, path2, action2, 2, flagIndex2, planIndex2
                    )
                    self.action2 = action2
                    self.done2 = done2
    ...
    def printShit(self, pos, agent):
        if agent == 1:
            ...
            plan = self.plan2
        return (
            self.getStepEasy(agent, pos, plan, False),
            self.world.roomsIndexToName(self.world.getCurrentRoom(pos)),
            self.world.flagsIndexToName(self.getFlagsGot(agent, pos)),
        )
    ...
    def runAgent(self, pos, Qas, path, action, agent, flagIndex, planIndex):
        ...
        sigma, nextAction, p, nextFlagIndex, nextPlanIndex = self.getSigma(
            reward, nextPos, pos, action, Qas, agent, flagIndex, planIndex
        )
    ...
    def chooseAction(self, pos, Qas, agent, flagsIndex, planIndex):
        ...
        if agent == 1 and not self.done2:
            posToAvoid.append(
                self.getNextPos(self.agent2[0], self.agent2[1], self.action2)
            )
        elif agent == 2 and not self.done1:
            posToAvoid.append(
                self.getNextPos(self.agent1[0], self.agent1[1], self.action1)
            )
        availableMoves = self.world.canMoveAvoid(pos, posToAvoid)
        if self.probability(self.epsilon):
            return choice(availableMoves), "random"
        else:
            maxValue, moves = self.computeMax(
                pos, availableMoves, Qas, flagsIndex, planIndex
            )
    ...
    def updateEligibilityTrace(self, path, sigma, Qas):
        ...
        for i in range(size - 1, size - cells - 1, -1):
            ...
            x, y = pos
            Qas[y][x][flagsIndex][planIndex][a] += (
                self.alpha * sigma * (self.gamma * self.lambd) ** (size - 1 - i)
            )

        return Qas

    def getSigma(
        self, reward, nextPos, currentPos, action, Qas, agent, flagIndex, planIndex
    ):
        ...
        nextAction, p = self.chooseAction(
            nextPos, Qas, agent, nextFlagIndex, nextPlanIndex
        )

        nextQ = Qas[nextPos[1]][nextPos[0]][nextFlagIndex][nextPlanIndex][nextAction]
        rS = self.rewardShaping(currentPos, nextPos, agent)
        sigma = reward + (self.gamma * nextQ) - originalQ + rS
    ...
    def phi(self, pos, agent):
        ...
        if self.world.isOnGoal(pos):
            ...
            return omega * (
                self.getStepEasy(agent, pos, plan, True) + self.testFlags(pos, agent)
            )
...
if __name__ == "__main__":
    ...
    marl = rsMARL(
        w, epsilon, gamma, alpha, lambd, start, planSolo, False, 0.0, 2000, True
    )
    marl.run()
